...
​	1.ELF文件具有结构化的特点，包含文件头、程序头表、节头表等元数据

​	2.binary文件就只有连续的字节序列，没有结构化的文件头或元数据

而ch2使用二进制文件的原因就是在批处理中，用户程序都使用了相同的加载地址`0x80400000`(tg-user/cases.toml第二行)，不需要复杂的ELF解析器，而且二进制文件比ELF文件小很多，使整个批处理程序运行更加流畅。
