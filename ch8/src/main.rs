    ...
    impl SyncMutex for SyscallContext {
        ...
        fn semaphore_create(&self, _caller: Caller, res_count: usize) -> isize {
                ...
                .find(|(_, item)| item.is_none()).map(|(id, _)| id)
            {
                ...
                // 确保 semaphore_total 有足够大小
                if id >= current_proc.semaphore_total.len() {
                    current_proc.semaphore_total.resize(id + 1, 0);
                }
                current_proc.semaphore_total[id] = res_count;
                id
            } else {
                current_proc.semaphore_list.push(Some(Arc::new(Semaphore::new(res_count))));
                current_proc.semaphore_total.push(res_count);
        ...
        fn semaphore_up(&self, _caller: Caller, sem_id: usize) -> isize {
            ...
            if current_proc.deadlock_detect_enabled
                && tid.get_usize() < current_proc.allocation_sem.len()
                && sem_id < current_proc.allocation_sem[tid.get_usize()].len()
                && current_proc.allocation_sem[tid.get_usize()][sem_id] > 0 {
                current_proc.allocation_sem[tid.get_usize()][sem_id] -= 1;
            }
        ...
        fn semaphore_down(&self, _caller: Caller, sem_id: usize) -> isize {
            ...
            if current_proc.deadlock_detect_enabled {
                // 确保矩阵大小足够
                let num_threads = tid.get_usize() + 1;
                let num_sems = current_proc.semaphore_list.len();

                while current_proc.allocation_sem.len() < num_threads {
                    current_proc.allocation_sem.push(vec![0; num_sems]);
                    current_proc.need_sem.push(vec![0; num_sems]);
                }
                for thread_alloc in current_proc.allocation_sem.iter_mut() {
                    while thread_alloc.len() < num_sems {
                        thread_alloc.push(0);
                    }
                }
                for thread_need in current_proc.need_sem.iter_mut() {
                    while thread_need.len() < num_sems {
                        thread_need.push(0);
                    }
                }

                // 标记需求增加 1
                current_proc.need_sem[tid.get_usize()][sem_id] += 1;

                // 执行死锁检测算法（信号量版本）
                let n_threads = num_threads;
                let n_sems = num_sems;

                // Available: 计算每个信号量的可用资源
                let mut available = Vec::new();
                for sem_id in 0..n_sems {
                    // 从 semaphore_total 获取总资源数
                    let total = if sem_id < current_proc.semaphore_total.len() {
                        current_proc.semaphore_total[sem_id]
                    } else {
                        0
                    };
                    // 计算已分配的资源
                    let mut allocated = 0;
                    for tid_idx in 0..n_threads {
                        allocated += current_proc.allocation_sem[tid_idx][sem_id];
                    }
                    // 可用 = 总数 - 已分配
                    available.push(total.saturating_sub(allocated));
                }
                ...
                loop {
                    ...
                    for tid_idx in 0..n_threads {
                        ...
                        for sid in 0..n_sems {
                            if current_proc.need_sem[tid_idx][sid] > work[sid] {
                                can_satisfy = false;
                                break;
                            }
                        }
                        ...
                        if can_satisfy {
                            ...
                            finish[tid_idx] = true;
                            for sid in 0..n_sems {
                                work[sid] += current_proc.allocation_sem[tid_idx][sid];
                            }
                            found = true;
                            break;
                        }
            ...
            // 更新分配矩阵
            if result && current_proc.deadlock_detect_enabled {
                current_proc.allocation_sem[tid.get_usize()][sem_id] += 1;
                current_proc.need_sem[tid.get_usize()][sem_id] -= 1;
            }
        ...
        fn mutex_unlock(&self, _caller: Caller, mutex_id: usize) -> isize {
            ...
            // 更新分配矩阵
            if current_proc.deadlock_detect_enabled
                && tid.get_usize() < current_proc.allocation_mutex.len()
                && mutex_id < current_proc.allocation_mutex[tid.get_usize()].len() {
                current_proc.allocation_mutex[tid.get_usize()][mutex_id] = false;
            }
        ...
        fn mutex_lock(&self, _caller: Caller, mutex_id: usize) -> isize {
            ...
            if current_proc.deadlock_detect_enabled {
                ...
                while current_proc.allocation_mutex.len() < num_threads {
                    current_proc.allocation_mutex.push(vec![false; num_mutexes]);
                    current_proc.need_mutex.push(vec![false; num_mutexes]);
                }
                for thread_alloc in current_proc.allocation_mutex.iter_mut() {
                    while thread_alloc.len() < num_mutexes {
                        thread_alloc.push(false);
                    }
                }
                for thread_need in current_proc.need_mutex.iter_mut() {
                    while thread_need.len() < num_mutexes {
                        thread_need.push(false);
                    }
                }

                // 标记需求
                current_proc.need_mutex[tid.get_usize()][mutex_id] = true;

                // 执行死锁检测算法
                let n_threads = num_threads;
                let n_mutexes = num_mutexes;

                // Available: 哪些 mutex 当前没有被任何线程持有
                let mut available = vec![true; n_mutexes];
                for tid_idx in 0..n_threads {
                    for mid in 0..n_mutexes {
                        if current_proc.allocation_mutex[tid_idx][mid] {
                            available[mid] = false;
                        }
                    }
                }
                ...
                loop {
                    ...
                    for tid_idx in 0..n_threads {
                        ...
                        let mut can_satisfy = true;
                        for mid in 0..n_mutexes {
                            if current_proc.need_mutex[tid_idx][mid] && !available[mid] {
                                can_satisfy = false;
                                break;
                            }
                        }
                ...
                if !finish.iter().all(|&f| f) {
                    // 检测到死锁
                    current_proc.need_mutex[tid.get_usize()][mutex_id] = false;
                    return -0xdead;
                }
            }
            ...
            // 更新分配矩阵
            if result && current_proc.deadlock_detect_enabled {
                current_proc.allocation_mutex[tid.get_usize()][mutex_id] = true;
                current_proc.need_mutex[tid.get_usize()][mutex_id] = false;
            }
