...
pub struct Process {
    ...
    /// Mutex 分配矩阵：allocation_mutex[thread_id][mutex_id] 表示线程是否持有该 mutex
    pub allocation_mutex: Vec<Vec<bool>>,
    /// Mutex 需求矩阵：need_mutex[thread_id][mutex_id] 表示线程是否需要该 mutex
    pub need_mutex: Vec<Vec<bool>>,
    /// Semaphore 分配矩阵：allocation_sem[thread_id][sem_id] 表示线程持有的信号量数量
    pub allocation_sem: Vec<Vec<usize>>,
    /// Semaphore 需求矩阵：need_sem[thread_id][sem_id] 表示线程还需要的信号量数量
    pub need_sem: Vec<Vec<usize>>,
    /// Semaphore 总资源数：记录每个信号量创建时的初始资源数
