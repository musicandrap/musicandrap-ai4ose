...
extern "C" fn rust_main() -> ! {
    ...
    // 增大调度栈以容纳更大的 Process 结构体
    const PAGE: Layout =
        unsafe { Layout::from_size_align_unchecked(4 << Sv39::PAGE_BITS, 1 << Sv39::PAGE_BITS) };
    let pages = 4;
...
mod impls {
    ...
    impl Trace for SyscallContext {
        ...
        fn trace(
            ...
            match trace_request {
                // 0: 读取用户内存
                0 => {
                    // 检查地址是否在用户地址空间范围内（Sv39 用户空间：[0, 2^38)）
                    if id >= (1 << 38) {
                        return -1;
                    }
                    const READABLE: VmFlags<Sv39> = build_flags("R_V");
                    if let Some(ptr) = unsafe { PROCESSES.get_mut() }
                        .get_mut(caller.entity)
                        .unwrap()
                        .address_space
    ...
    impl Memory for SyscallContext {
        fn mmap(
            ...
            // 构建权限标志：U（用户态）+ prot
            // 格式: U D A G X W R V（8个字符）
            let mut flags_str = [b'U', b'_', b'_', b'_', b'V'];
            if prot & 0x1 != 0 { flags_str[3] = b'R'; } // 可读 (index 3)
            if prot & 0x2 != 0 { flags_str[2] = b'W'; } // 可写 (index 2)
